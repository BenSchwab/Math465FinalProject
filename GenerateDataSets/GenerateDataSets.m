...
%         % Create grid in [-1/2,1/2]^D with cOpts.NumberOfPoints points
%         ntauperdim = round(cOpts.NumberOfPoints^(1/cOpts.Dim));
%         tauperdim  = linspace(-1/2,1/2,ntauperdim);
%         str_eval = '[';
%         for k = 1:cOpts.Dim,
%             str_eval = [str_eval,sprintf('Xs_%d,',k)];
%         end
%         str_eval(end) = ']';
%         str_eval = [str_eval,'=ndgrid(tauperdim);'];
%         eval(str_eval);                       
        ...
        GraphDiffOpts = struct( ...
            ...
            'Epsilon',1, ...
            'kNN', 5, ...
            'kNNAutotune', 4*cOpts.Dim, ...
            'kEigenVecs', 10*cOpts.Dim, ...
            'Symmetrization', 'W+Wt', ...
            'DontReturnDistInfo', 1, ...
            'XIsTransposed', 1);
        % Set default options for constructing the multiscale svd nets
...
X = X';
if isfield(GraphDiffOpts,'XIsTransposed'),
    GraphDiffOpts=rmfield(GraphDiffOpts,'XIsTransposed');
end
if isfield(NetsOpts,'XIsTransposed'),
